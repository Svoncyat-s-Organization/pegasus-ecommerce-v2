...
import { formatPhone, cleanPhone } from '@shared/utils/formatters';
...
export const UserFormModal = ({ mode, userId, visible, onClose }: UserFormModalProps) => {
  ...
  const handleFinish = (values: CreateUserRequest | UpdateUserRequest) => {
    // Limpiar teléfono antes de enviar (remover formato)
    const cleanedValues = {
      ...values,
    ...
    if (mode === 'create') {
      createUser.mutate(cleanedValues as CreateUserRequest, {
        onSuccess: () => {
          form.resetFields();
          onClose();
        },
      ...
      updateUser.mutate(
        { id: userId, userData: cleanedValues as UpdateUserRequest },
        {
          onSuccess: () => {
            onClose();
          },
          ...
          rules={[
            {
              validator(_, value) {
                ...
                if (!/^9\d{8}$/.test(cleaned)) {
                  return Promise.reject(new Error('Debe ser 9 dígitos e iniciar con 9'));
                }
                return Promise.resolve();
              },
            },

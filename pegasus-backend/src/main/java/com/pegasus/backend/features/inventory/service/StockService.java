...
public class StockService {
     ...
     */
    public StockAvailabilityResponse checkStockAvailability(Long warehouseId, Long variantId, Integer requiredQuantity) {
        ...
        String message = isAvailable 
                ...
                : "Stock insuficiente. Disponible: " + availableQuantity + ", Requerido: " + requiredQuantity;
    ...
    public StockResponse adjustStock(AdjustStockRequest request, Long userId) {
        ...
        Warehouse warehouse = warehouseRepository.findById(request.warehouseId())
                .orElseThrow(() -> new ResourceNotFoundException("Almacén no encontrado con ID: " + request.warehouseId()));
        
        Variant variant = variantRepository.findById(request.variantId())
                .orElseThrow(() -> new ResourceNotFoundException("Variante no encontrada con ID: " + request.variantId()));
    ...
    public void transferStock(TransferStockRequest request, Long userId) {
        log.info("Transferring stock - variant: {}, from: {}, to: {}, quantity: {}", 
                request.variantId(), request.fromWarehouseId(), request.toWarehouseId(), request.quantity());
        ...
        Warehouse fromWarehouse = warehouseRepository.findById(request.fromWarehouseId())
                .orElseThrow(() -> new ResourceNotFoundException("Almacén de origen no encontrado con ID: " + request.fromWarehouseId()));
        
        Warehouse toWarehouse = warehouseRepository.findById(request.toWarehouseId())
                .orElseThrow(() -> new ResourceNotFoundException("Almacén de destino no encontrado con ID: " + request.toWarehouseId()));
        
        Variant variant = variantRepository.findById(request.variantId())
                .orElseThrow(() -> new ResourceNotFoundException("Variante no encontrada con ID: " + request.variantId()));
        
        // Obtener stock de origen
        Stock fromStock = stockRepository.findByWarehouseIdAndVariantId(request.fromWarehouseId(), request.variantId())
                .orElseThrow(() -> new ResourceNotFoundException(
                        "No existe stock en el almacén de origen para la variante " + request.variantId()));
        ...
        Stock toStock = stockRepository.findByWarehouseIdAndVariantId(request.toWarehouseId(), request.variantId())
                .orElseGet(() -> {
                    Stock newStock = Stock.builder()
        ...
        log.info("Stock transferred successfully - variant: {}, quantity: {}", request.variantId(), request.quantity());
    }
    ...
    public void reserveStock(Long warehouseId, Long variantId, Integer quantity, Long orderId, Long userId) {
        ...
        log.info("Stock reserved successfully - variant: {}, quantity: {}, order: {}", variantId, quantity, orderId);
    }
     ...
     */
    @Transactional
    public void releaseReservedStock(Long warehouseId, Long variantId, Integer quantity, Long orderId, Long userId) {
        ...
        log.info("Reserved stock released successfully - variant: {}, quantity: {}, order: {}", variantId, quantity, orderId);
    }

    /**
     * Decrementa stock físico al enviar un pedido (INTERNO - usado por OrderService)
     */
    @Transactional
    public void decreaseStock(Long warehouseId, Long variantId, Integer quantity, Long orderId, Long userId) {
        ...
        log.info("Stock decreased successfully - variant: {}, quantity: {}, order: {}", variantId, quantity, orderId);
    }
    ...
    public void increaseStock(Long warehouseId, Long variantId, Integer quantity, 
        ...
        log.info("Stock increased successfully - variant: {}, quantity: {}, purchase: {}", variantId, quantity, purchaseId);
    }
    ...
    public void returnStock(Long warehouseId, Long variantId, Integer quantity, Long rmaId, Long userId) {
        ...
        log.info("Stock returned successfully - variant: {}, quantity: {}, rma: {}", variantId, quantity, rmaId);
    }
}
